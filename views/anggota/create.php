...
$anggota_pegawai_id = array_column(Anggota::all(), 'pegawai_id');
$pegawaiTersedia = array_filter($pegawai, function ($p) use ($anggota_pegawai_id) {
    return !in_array($p['id'], $anggota_pegawai_id);
});
...
<form method="POST">
    ...
    <div class="mb-3">
        <select name="kartu_diskon_id" class="form-control" required>
            ...
            $kartuDiskonList = Kartu_diskon::all(); // pastikan method ini ada
            foreach ($kartuDiskonList as $diskon) {
                echo "<option value='{$diskon['id']}'>{$diskon['nama_diskon']}</option>";
            }
            ?>
        </select>
